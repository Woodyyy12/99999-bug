...
# ==========================================
# PARA FORMATLAMA YARDIMCISI
# ==========================================
def para_formatla(miktar):
    """Sayıyı Türkçe noktalı format ile gösterir. Örn: 100.000.000"""
    return f"{int(miktar):,}".replace(",", ".")
...
async def on_ready():
    ...
    bot.add_view(KayitView())   # Persistent kayıt butonları — restart'tan sonra da çalışır
...
_son_katilan = {}  # çift mesaj önleme: {user_id: timestamp}

@bot.event
async def on_member_join(member: discord.Member):
    simdi = time.time()
    if simdi - _son_katilan.get(member.id, 0) < 10:
        return  # 10 saniye içinde aynı üye için tekrar çalışma
    _son_katilan[member.id] = simdi
    ...
    if bildirim_kanal is None:
        try:
            bildirim_kanal = await bot.fetch_channel(KAYIT_BILDIRIM_KANAL_ID)
        except Exception as e:
            print(f"❌ Bildirim kanalı bulunamadı: {e}")
            return
    ...
    try:
        ...
        print(f"✅ Kayıt bildirimi gönderildi: {member.name}")
    except discord.Forbidden:
        print(f"❌ Kanal izin hatası: Bot #{KAYIT_BILDIRIM_KANAL_ID} kanalına mesaj atamıyor.")
    except Exception as e:
...
# ==========================================
# KAYIT BUTON SİSTEMİ — Persistent (bot restart'a dayanıklı)
# ==========================================
ROL_UYE       = 1522570902473019552
ROL_FUTBOLCU  = 1523967349932294234
ROL_TEKNIK_D  = 1522570884462809179
ROL_EKSTRA    = 1522570902473019552  # Her kayıtta ekstradan verilecek rol

# Bekleyen kayıtlar dosyası — bot restart'ta kaybedilmez
BEKLEYEN_KAYITLAR_DOSYASI = os.path.join(BASE_DIR, "bekleyen_kayitlar.json")

def bekleyen_kayitlar_yukle():
    if os.path.exists(BEKLEYEN_KAYITLAR_DOSYASI):
        with open(BEKLEYEN_KAYITLAR_DOSYASI, "r", encoding="utf-8") as f:
            try:
                return json.load(f)
            except json.JSONDecodeError:
                return {}
...
def bekleyen_kayitlar_kaydet(veri):
    with open(BEKLEYEN_KAYITLAR_DOSYASI, "w", encoding="utf-8") as f:
        json.dump(veri, f, ensure_ascii=False, indent=4)

bekleyen_kayitlar = bekleyen_kayitlar_yukle()


async def kayit_rol_ver(interaction: discord.Interaction, uye_id: int, rol_id: int, rol_adi: str):
    """Kayıt işlemini tamamlar. Persistent butonlardan çağrılır."""

    # Yetkili kontrolü: sadece Kayıt Yetkilisi veya Admin basabilir
    kayit_yetkilisi_rol = discord.utils.get(interaction.guild.roles, name="Kayıt Yetkilisi")
    kullanici = interaction.user
    yetkili_mi = (
        kullanici.guild_permissions.administrator
        or (kayit_yetkilisi_rol and kayit_yetkilisi_rol in kullanici.roles)
    )
    if not yetkili_mi:
        await interaction.response.send_message(
            "❌ Bu butonu sadece **Kayıt Yetkilisi** veya adminler kullanabilir!", ephemeral=True
        )
        return

    # Bekleyen kayıt verisini al
    uid_str = str(uye_id)
    veri = bekleyen_kayitlar.get(uid_str)
    if not veri:
        await interaction.response.send_message(
            "❌ Bu kayıt formu artık geçerli değil. Yeniden `.k @kullanıcı <isim>` komutu kullanın.", ephemeral=True
        )
        return

    yeni_isim = veri["isim"]
    uye = interaction.guild.get_member(uye_id)
    if not uye:
        try:
            uye = await interaction.guild.fetch_member(uye_id)
        except Exception:
            await interaction.response.send_message(
                "❌ Üye sunucuda bulunamadı. Sunucudan ayrılmış olabilir.", ephemeral=True
            )
            return
    ...
    for item in interaction.message.components:
        for comp in item.children if hasattr(item, 'children') else [item]:
            pass  # Discord.py otomatik olarak view'ı günceller
    ...
    # Bekleyen kayıttan sil
    bekleyen_kayitlar.pop(uid_str, None)
    bekleyen_kayitlar_kaydet(bekleyen_kayitlar)
    ...
    try:
        ...
        nick_hata = "⚠️ Nick değiştirilemedi (botun rolü üyeden altta veya sunucu sahibi)."
    except Exception as e:
        nick_hata = f"⚠️ Nick hatası: {e}"

    # --- 3. Rolleri ver ---
    rol_hata = None
    try:
        rol = interaction.guild.get_role(rol_id)
        ekstra_rol = interaction.guild.get_role(ROL_EKSTRA)
        kayitsiz_rol = discord.utils.get(interaction.guild.roles, name="Kayıtsız Üye")

        if rol:
            await uye.add_roles(rol)
        if ekstra_rol and ekstra_rol not in uye.roles:
            await uye.add_roles(ekstra_rol)
        if kayitsiz_rol and kayitsiz_rol in uye.roles:
            await uye.remove_roles(kayitsiz_rol)
    except discord.Forbidden:
        rol_hata = "⚠️ Rol verilemedi (botun rolü en üste alınmalı)."
    except Exception as e:
        rol_hata = f"⚠️ Rol hatası: {e}"

    # --- 4. Sonuç embed ---
    embed = discord.Embed(title="✅ Kayıt Tamamlandı!", color=discord.Color.green())
    embed.description = (
        f"🏃‍♂️ {uye.mention} Ruthless League lisansı aldı!\n\n"
        f"📋 **İsim:** `{yeni_isim}`\n"
        f"🏅 **Rol:** `{rol_adi}`\n"
        f"🛡️ **İşlemi Yapan:** {kullanici.mention}"
    )
    if nick_hata:
        embed.add_field(name="⚠️ Uyarı", value=nick_hata, inline=False)
    if rol_hata:
        embed.add_field(name="⚠️ Uyarı", value=rol_hata, inline=False)
    embed.set_thumbnail(url=uye.avatar.url if uye.avatar else uye.default_avatar.url)

    # Butonları devre dışı bırakılmış view ile mesajı güncelle
    disabled_view = KayitView(uye_id=uye_id)
...
class KayitView(discord.ui.View):
    """Persistent kayıt view — bot restart'tan sonra da çalışır."""

    def __init__(self, uye_id: int = 0):
        super().__init__(timeout=None)  # Süresiz — dosyadan silene kadar aktif
        self.uye_id = uye_id
    ...
    async def uye_buton(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Hangi üye için bu mesaj açıldıysa onu bul
        uye_id = self._mesajdan_uye_id(interaction)
        if uye_id is None:
            await interaction.response.send_message("❌ Kayıt verisi bulunamadı.", ephemeral=True)
            return
        await kayit_rol_ver(interaction, uye_id, ROL_UYE, "Üye")
    ...
    async def futbolcu_buton(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if uye_id is None:
            await interaction.response.send_message("❌ Kayıt verisi bulunamadı.", ephemeral=True)
            return
        await kayit_rol_ver(interaction, uye_id, ROL_FUTBOLCU, "Futbolcu")

    @discord.ui.button(label="Teknik D.", style=discord.ButtonStyle.secondary, emoji="📋",
                       custom_id="kayit_teknikd")
    async def teknik_buton(self, interaction: discord.Interaction, button: discord.ui.Button):
        uye_id = self._mesajdan_uye_id(interaction)
        if uye_id is None:
            await interaction.response.send_message("❌ Kayıt verisi bulunamadı.", ephemeral=True)
            return
        await kayit_rol_ver(interaction, uye_id, ROL_TEKNIK_D, "Teknik Direktör")

    def _mesajdan_uye_id(self, interaction: discord.Interaction) -> int | None:
        """Mesaj embed'inden üye ID'sini bekleyen_kayitlar'dan bulur."""
        # Mesaj ID'si ile bekleyen kayıttan üye bul
        msg_id = str(interaction.message.id)
        for uid, veri in bekleyen_kayitlar.items():
            if veri.get("mesaj_id") == msg_id:
                return int(uid)
        return None


# ==========================================
# KAYIT KOMUTU: .k @Kullanici İsim
# ==========================================
@bot.command(name="k")
async def kayit(ctx, uye: discord.Member = None, *, yazi: str = None):
    # Yetkili kontrolü: Admin veya Kayıt Yetkilisi rolü
    kayit_yetkilisi_rol = discord.utils.get(ctx.guild.roles, name="Kayıt Yetkilisi")
    yetkili_mi = (
        ctx.author.guild_permissions.administrator
        or (kayit_yetkilisi_rol and kayit_yetkilisi_rol in ctx.author.roles)
    )
    if not yetkili_mi:
        await ctx.send("❌ Bu komutu sadece **Kayıt Yetkilisi** rolüne sahip kişiler veya adminler kullanabilir!")
        return
    ...
    # Bekleyen kayıt oluştur (mesaj ID sonradan güncellenecek)
    uid_str = str(uye.id)
    bekleyen_kayitlar[uid_str] = {
        "isim": yazi,
        "mesaj_id": None  # Mesaj gönderildikten sonra güncellenecek
    }
    ...
    embed.set_thumbnail(url=uye.avatar.url if uye.avatar else uye.default_avatar.url)
    embed.set_footer(text=f"Komutu kullanan: {ctx.author.display_name} • Ruthless League")

    view = KayitView(uye_id=uye.id)
    mesaj = await ctx.send(embed=embed, view=view)

    # Mesaj ID'sini bekleyen kayıta kaydet
    bekleyen_kayitlar[uid_str]["mesaj_id"] = str(mesaj.id)
    bekleyen_kayitlar_kaydet(bekleyen_kayitlar)


# ==========================================
# KAYIT SİL KOMUTU: .kver @kullanici
# ==========================================
...
async def profil(ctx, uye: discord.Member = None):
    ...
    embed.add_field(name="💰 Piyasa Değeri", value=f"`{para_formatla(p['para'])}`", inline=True)
...
async def penalti(ctx):
        ...
        try:
            ...
            if kalan_sure.total_seconds() > 0:
                ...
                await ctx.send(embed=discord.Embed(
                    ...
                    description=f"<a:saat:1545088358353731705> Penaltı hakkın henüz yenilenmedi.\n\nYeni hakkını **{dakika} dakika** sonra kullanabilirsin.",
                ))
                return
        except ValueError:
    ...
    senaryolar = [
        {"mesaj": "<:penalti:1545845312306942002> PENALTI GOOOOLLLL<a:peepoclap:1544858466752008263>\n\n<a:BTFS_ZirveElmas:1544843346479161424> Değer Kazancı 2M\n\n<a:saat:1545088358353731705> Sonraki Hakkın 1 Saat Sonra.", "gol": True},
        {"mesaj": "<:penaltikacti:1544828350969942107> VURUŞŞ AUT <:emoji_130:1545847240357060731>\n\n<:popeyes62:1544843272705540246> Değer Kazancı 0\n\n<a:saat:1545088358353731705> Üzülme 1 Saat Sonra Yeniden Kullanabilirsin.", "gol": False},
        {"mesaj": "<:penalti:1545845309945421854> VEEE DİREEKK <:baggio:1545846769353629706>\n\n<:popeyes62:1544843272705540246> Değer Kazancı 0\n\n<a:saat:1545088358353731705> Ayakta Kalmanın Manası Yok 1 Saat Sonra Yeniden Burada Olacaksın.", "gol": False},
        {"mesaj": "<:penaltikacti:1544828350969942107> GÜÇSÜZ BİR VURUŞ VE KALECİ <:kurtaris:1545849244173344798>\n\n<:popeyes62:1544843272705540246> Değer Kazancı 0\n\n<a:saat:1545088358353731705> 1 Saat İçinde O Şişeyi İmha Edip Geri Gel.", "gol": False},
    ]
...
ROL_KAP_YETKILI = 1522570884462809179  # KAP işlemi yapabilecek rol
...
class KapFormView(discord.ui.View):
    ...
    async def devam(self, interaction: discord.Interaction, button: discord.ui.Button):
        kap_rol = interaction.guild.get_role(ROL_KAP_YETKILI)
        if not kap_rol or kap_rol not in interaction.user.roles:
            await interaction.response.send_message("❌ Bu formu sadece yetkili rolüne sahip kişiler doldurabilir!", ephemeral=True)
            return
        if not self.kap_turu or self.eski_kulup_rol is None or not self.yeni_kulup_rol:
...
async def kap(ctx, uye: discord.Member = None):
    kap_rol = discord.utils.get(ctx.guild.roles, id=ROL_KAP_YETKILI)
    if not kap_rol or kap_rol not in ctx.author.roles:
        await ctx.send("❌ Bu komutu sadece yetkili rolüne sahip kişiler kullanabilir.", delete_after=5)
        await ctx.message.delete()
        return
    if uye is None:
...
async def takim_info(ctx, *, takim_adi: str = None):
    ...
    embed.add_field(
        name="💰 Toplam Bütçe",
        value=f"`{para_formatla(toplam_butce)}`",
        inline=True
    )
    embed.add_field(
        name="📈 Takım Değeri",
        value=f"`{para_formatla(toplam_deger)}`" if toplam_deger else "`—`",
        inline=True
    )
    embed.add_field(
        name="📋 Kadro",
...
async def para_goster(ctx):
    ...
    embed.add_field(name="💵 Mevcut Para", value=f"**{para_formatla(p['para'])}**", inline=False)
...
async def para_gonder(ctx, miktar: int = None, hedef: discord.Member = None):
    ...
    if gonderen["para"] < miktar:
        await ctx.send(f"❌ Yetersiz bakiye! Elinde sadece **{para_formatla(gonderen['para'])}** var.")
        return
    ...
    embed.add_field(name="📤 Gönderen", value=f"{ctx.author.mention}\n*Yeni bakiye: {para_formatla(gonderen['para'])}*", inline=True)
    embed.add_field(name="💰 Miktar",   value=f"**{para_formatla(miktar)}**",                                             inline=True)
    embed.add_field(name="📥 Alan",     value=f"{hedef.mention}\n*Yeni bakiye: {para_formatla(alici['para'])}*",          inline=True)
    embed.set_footer(text="⚽ Ruthless League • Para Sistemi")
    await ctx.send(embed=embed)


@bot.command(name="paraekle")
async def para_ekle(ctx, miktar: int = None, hedef: discord.Member = None):
    """TAC rolü: kimseden almadan para ekler. .paraekle <miktar> @kullanici"""
    ...
    embed.add_field(name="➕ Eklenen",     value=f"**{para_formatla(miktar)}**",                          inline=True)
    embed.add_field(name="💵 Yeni Bakiye", value=f"**{para_formatla(oyuncular[alici_id]['para'])}**",     inline=True)
    embed.set_footer(text=f"İşlemi yapan: {ctx.author.display_name} • Ruthless League")
    await ctx.send(embed=embed)
...
# YARDIM KOMUTU
# ==========================================
@bot.command(name="yardım", aliases=["yardim", "yrd", "komutlar"])
async def yardim(ctx):
    """Tüm komutları listeler."""

    embed = discord.Embed(
        title="⚽ Ruthless League — Komut Listesi",
        description="Tüm komutlar `.` prefix'i ile kullanılır.\nÖrnek: `.profil` veya `.ant`",
        color=discord.Color.blurple()
    )
    embed.set_footer(text="⚽ Ruthless League • Yardım Sistemi")
    embed.set_author(
        name="Ruthless League Bot",
    ...
    embed.add_field(
        ...
        value=(
            "`.profil [@kullanıcı]` — Profilini veya başkasının profilini görüntüle\n"
            "`.ant` — Antrenman yap (+5 para, 1 saatte bir)\n"
            "`.penaltı` — Penaltı çek (şans oyunu)\n"
        ),
        inline=False
    )
    ...
    embed.add_field(
        name="💰 Para Komutları",
        value=(
            "`.para` — Mevcut para bakiyeni gör\n"
            "`.parag <miktar> @kullanıcı` — Birine para gönder\n"
            "`.dver @kullanıcı <miktar> [sebep]` — Futbolcuya M€ değeri ekle *(Kayıt Yetkilisi)*\n"
            "`.paraekle <miktar> @kullanıcı` — Birinin bakiyesine para ekle *(TAC rolü)*\n"
        ),
        inline=False
    )

    embed.add_field(
        name="🏟️ Takım & Transfer Komutları",
        value=(
            "`.takim <takım adı>` — Takım kadrosunu ve bilgilerini gör\n"
            "`.kap @kullanıcı` — KAP transfer bildirimi başlat *(Yetkili)*\n"
        ),
        inline=False
    )

    embed.add_field(
        name="📋 Kayıt Komutları *(Admin)*",
        value=(
            "`.k @kullanıcı <isim>` — Üyeyi sisteme kaydet, rol seçimi yapar\n"
            "`.kver @kullanıcı` — Üyenin kaydını sil, tüm rollerini al\n"
        ),
        inline=False
    )

    embed.add_field(
        name="🛡️ Moderasyon Komutları *(Yetkili)*",
        value=(
            "`.mute @kullanıcı [dakika] [sebep]` — Kullanıcıyı sustur (varsayılan: 10 dk)\n"
            "`.unmute @kullanıcı` — Susturmayı kaldır\n"
            "`.kick @kullanıcı [sebep]` — Kullanıcıyı sunucudan at\n"
            "`.ban @kullanıcı [sebep]` — Kullanıcıyı banla\n"
            "`.unban <kullanıcı_id>` — Banlı kullanıcının banını kaldır\n"
        ),
        inline=False
    )

    embed.add_field(
        name="⚙️ Kurulum Komutları *(Admin)*",
        value=(
            "`.tkur` — Ticket (destek talebi) sistemini kur\n"
            "`.sjbj` — Oto-rol mesajını kur\n"
        ),
        inline=False
    )
...
if not TOKEN:
    ...
    print("   Railway > Variables kısmına 'DISCORD_TOKEN' adıyla token'ı ekle.")
    exit(1)

try:
    bot.run(TOKEN)
except discord.LoginFailure:
    print("❌ HATA: Token geçersiz! Discord Developer Portal'dan token'ı kontrol et.")
    exit(1)
except Exception as e:
